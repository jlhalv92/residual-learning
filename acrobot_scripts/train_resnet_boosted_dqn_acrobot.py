...
from src.networks_discrete import Q0,  Q1
...
def experiment(alg,
    ...
    old_q = Agent.load(prior_path)

    pi = EpsGreedy(epsilon=epsilon_random, old_q=old_q, explore_old_steps=2*initial_replay_size, boosting=True, explore_old=False)

    agent = alg(mdp.info, pi, NumpyTorchApproximator, approximator_params=approximator_params, **algorithm_params)
    core = Core(agent, mdp)

    if boosting:
        agent.setup_boosting(old_q)
    ...
    # core.evaluate(n_episodes=1, render=True)tr
...
if __name__ == '__main__':
    ...
    for task in [3.]:
        for i in range(seed):
            model_name = "agent_resnet_boosted_residual_task_m={}_kg_gradient_steps=1".format(task)
