...
import os
from src.algorithms.value_based.residual_dqn import ResNetBoostedDQN
...
from src.networks_discrete import Q0,  Q1, Q2, Q3, Q4, Q5
...
def experiment(alg,
    ...
    if boosting:
        print(prior_path)
        rho = Agent.load(prior_path)
        agent.setup_boosting(rho)
...
if __name__ == '__main__':
    ...
    for i in range(seed):
        for task_id, task in enumerate(tasks):
            if task_id == 0:
                ...
                boosting=True
            model_name = "agent_restnet_task_m={}_kg".format(task)
            experiment(alg=ResNetBoostedDQN,
                       n_epochs=30,
                       network=nets[task_id],
                       logging=True,
                       model_name="{}_{}".format(model_name,i),
                       model_name_logging="{}".format(model_name),
                       model_dir="../src/checkpoint/Acrobot/RESNET",
                       n_steps=5000,
                       n_episodes=10,
                       prior_path="src/checkpoint/Acrobot/RESNET/agent_restnet_task_m={}_kg_{}".format(tasks[task_id-1],i),
                       task=task,
                       boosting=boosting,
    ...
    #     for i in range(seed):
    #
    #         model_name = "agent_restnet_task_m={}_kg_explore_old_policy".format(task)
    #
    #         experiment(alg=ResNetBoostedDQN,
    #                    n_epochs=30,
    #                    network=Q1,
    ...
    #                    n_steps=5000,
    #                    n_episodes=10,
    #                    prior_path="src/checkpoint/Acrobot/Nominal/agent_restnet_BASE_0_0",
    #                    task=task,
    #                    boosting=True,
