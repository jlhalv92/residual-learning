...
class SACPolicy(Policy):
    ...
    the internals calculations of the actor_critic algorithm.
...
class SAC(DeepAC):
    ...
    def __init__(self, mdp_info, actor_mu_params, actor_sigma_params,
        ...
        self._residual = to_parameter(False)
        ...
        ensemble_target_critic_params = deepcopy(critic_params)


        self._critic_approximator = Regressor(TorchApproximator,
                                              **critic_params)

        ensemble_target_critic_params['n_models'] = 4
        self._ensemble_target_critic_approximator = Regressor(TorchApproximator,
                                                     **ensemble_target_critic_params)

        # nominal_critic_params['n_models'] = 4
        # self._nominal_critic_approximator = Regressor(TorchApproximator,
        #                                              **nominal_critic_params)




        # set_weights_to_zero(self._nominal_critic_approximator[0].network)
        # set_bias_to_zero(self._nominal_critic_approximator[0].network)
        # freeze_weights(self._nominal_critic_approximator[0].network)
        #
        # set_weights_to_zero(self._nominal_critic_approximator[1].network)
        # set_bias_to_zero(self._nominal_critic_approximator[1].network)
        # freeze_weights(self._nominal_critic_approximator[1].network)
        ...
        self._init_target(self._critic_approximator,
                          self._ensemble_target_critic_approximator)
        ...
        self._add_save_attr(
            ...
            _ensemble_target_critic_approximator='mushroom',
            _log_alpha='torch',
            _alpha_optim='torch',
            _residual="mushroom",
        )
    ...
    def _init_target(online, target):
        for i in range(len(online)):
            ...
            target[i+2].set_weights(online[i].get_weights())
    ...
    def _next_q(self, next_state, absorbing):
        ...
        ensemble_q_target_array = self._ensemble_target_critic_approximator.predict(next_state, a, prediction='all')

        if self._residual():
            ensemble_q_target_sum_array = ensemble_q_target_array[:-2, :] + ensemble_q_target_array[2:, :]
            q_target = ensemble_q_target_sum_array.min(0)

        else:
            q_target = ensemble_q_target_array[:2,].min(0)

        q = q_target - self._alpha_np * log_prob_next
        q *= 1 - absorbing

        return q

    def _q_nominal(self, state, action):
